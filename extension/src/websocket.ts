...
export async function activate(
  ...
  outputChannel = vscode.window.createOutputChannel("CodeDock");
  events = new EventEmitter();

  const apiClient = new ApiClient(serverUrl);
  authManager = new AuthManager(context.secrets, apiClient, events);
...
async function handleJoinRoom(): Promise<void> {
  ...
  wsManager.connect(token, roomId.trim());
  yjsSync.activate();
